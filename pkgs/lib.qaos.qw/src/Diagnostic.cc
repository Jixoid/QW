...
namespace qw::diagnostic
{
  ...
  inline fun format_from_vector(const string &fmt_str, const vector<string> &v) -> string
  {
    switch (v.size()) {
    ...
    if (msg->pos()) {
      ...
      for (u0 i = Beg; i <= End; i++)
      {
        ...
        if (c == '\n' || i == End) 
        {
          ...
          os << "  " << string(padd - std::to_string(line).length() + 1, ' ') 
      ...
      {
        if (i == Beg || file[i] == '\n') {
          os << '\n' << "  " << string(padd-log10(line)+1, ' ') << line << color::GRAY << " | " << color::RESET;
      ...
      //os << "  " << string(log10(msg->pos()->HY)+1, ' ') << color::GRAY << " | " << color::RESET << string(msg->pos()->HX, ' ') << color::B_GREEN << "^" << string(msg->pos()->Size-1, '~') << color::RESET << endl;
      
      //Beg = (Beg == string_view::npos) ? 0 : Beg+1;
      //if (End == string_view::npos) End = msg->pos().file().size();
  ...
  fun operator <<(ostream &os, qSummary &sum) -> ostream&
  {
    string out;
    
    if (auto x = sum.fatal())   out += to_string(x) + " " + _("fatal") + ", ";
    if (auto x = sum.error())   out += to_string(x) + " " + _("error") + ", ";
    if (auto x = sum.warning()) out += to_string(x) + " " + _("warning") + ", ";
    if (auto x = sum.hint())    out += to_string(x) + " " + _("hint") + ", ";
    if (auto x = sum.note())    out += to_string(x) + " " + _("note") + ", ";


    return os
      << (sum.fatal() ? color::RED : color::YELLOW)
        << _("summary") << color::GRAY << ":" << color::RESET << " "
      << string_view(out).substr(0, out.size()-2)
      << " " + _("generated.") << endl;
  }
  ...
  [[gnu::noreturn]] fun fatal(string msg) -> void
  {
    cerr << "qw encountered a fatal error:" << endl;
