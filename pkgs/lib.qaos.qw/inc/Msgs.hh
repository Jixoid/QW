...
#define NewMsg(Name, Type, Msg, N) \
  inline fun Name(word w REPN(N, string p)) { return unexpected(diagnostic:: Type ::make(w, Msg, {REP(N, p)} )); }

#define NewMsgF(Name, Msg, N) \
  inline fun Name(REPN(N, string p)) { return unexpected(diagnostic::qFatal::make(Msg, {REP(N, p)} )); }
...
namespace qw::errors
{
  ...
  NewMsg(UnExpectedIdentifier, qError, _("unexpected identifier ‘<blue>{}<reset>‘."), 1);
