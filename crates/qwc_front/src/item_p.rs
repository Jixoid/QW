...
use crate::{front::Ctx, ctx, ExprParser, MetaParser, TypeParser, WordCheck};
...
impl ItemParser {
  ...
  fn sub_let(ctx: &mut Ctx, vis: Visibility) -> Result<ItemId, Message> { ctx!(ctx => cre, sin, far, lex, sum);
    ...
    let value = match lex.get_k()? {
      (WK::Assign, _) => {
        let a = Some(ExprParser::read_expr(ctx!(cre, sin, far, lex, sum))?);
        lex.get()?.expect_kind(WK::Semicolon)?;
        a
      }
      (WK::Semicolon, _) => None,
      
      (_, c) => c.panic_kind2(WK::Assign, WK::Semicolon)?
    };
  ...
  fn sub_let_in(ctx: &mut Ctx, vis: Visibility) -> Result<ItemId, Message> { ctx!(ctx => cre, sin, far, lex, sum);
    ...
    let this = Item{
      ...
      vis,
      name: Some(name),
      kind: ItemKind::Let{kind, value, ism: true}
    };
  ...
  fn sub_generic(ctx: &mut Ctx, mut vis: Visibility) -> Result<ItemId, Message> { ctx!(ctx => cre, sin, far, lex, sum);
    ...
    let params = {
      ...
      loop {
        if lex.peek()?.kind() == WK::AngleEnd { lex.bump()?; break; }
        
        let mut names = vec![];
        loop {
          names.push(lex.get()?.ident(sin, far)?);
        ...
        for x in names {
          let this = Thing::NamedType(x, kind);
          
          params.push(cre.push(this));
        }
        
        match lex.get_k()? {
          (WK::Comma | WK::Semicolon, _) => continue,
          (WK::AngleEnd, _) => break,

          (_, c) => c.panic_kind3(WK::Comma, WK::Semicolon, WK::AngleEnd)?
        }
      }
